    ...
    public class CustomerGroupsController : BaseController
    {
           ...
           switch(submitButton) {
            ...
            case "Discard":
               //Cancel return to Index view
                   ...
                   Id = db.Sys_PartnerGroups.Where(t => t.MemberShipId == MemberShipId && t.CompanyId == CC && t.GroupType==1 &&  t.Deleted != true).Max(c => (int?)c.Id);
        ...
        protected Boolean Insert(Sys_PartnerGroups MainTbl) 
            {
                ...
                if (MainTbl.Id == 0)
                {
                 ...
                 if (MainTbl.Code == null)
                 {
                     ...
                     int Max = db.Database.SqlQuery<int>("select isnull(MAX(CAST(Code AS int)) ,0)+1 from dbo.Sys_PartnerGroups  WHERE (isnumeric(Code) = 1) and CompanyId=(" + MainTbl.CompanyId + ") and (GroupType=1) and  (MemberShipId = " + Session["MemberShipId"] + ") AND (ISNULL(Deleted, 0) = 0)").FirstOrDefault(); 
        ...
        protected Boolean Post(Sys_PartnerGroups MainTbl)
       {
        ...
        public ActionResult Search([DataSourceRequest]DataSourceRequest request, string TxtSearchVal, int? CompanySearchId)
        {
           ...
           {
               var m = db.Sys_PartnerGroups.Where(b => b.MemberShipId == MemberShipId && b.CompanyId==CompanySearchId && b.GroupType==1 && b.Deleted != true && (b.Code.Contains(TxtSearchVal) || b.NameE.Contains(TxtSearchVal) || b.NameA.Contains(TxtSearchVal) || b.Notes.Contains(TxtSearchVal))).Take(20);
           ...
           {
               var m = db.Sys_PartnerGroups.OrderByDescending(b => b.Id).Where(b => b.MemberShipId == MemberShipId && b.CompanyId == CompanySearchId && b.GroupType == 1 && b.Deleted != true).Take(20);
               IQueryable<Sys_PartnerGroups> mm = m;
               DataSourceResult result = mm.ToDataSourceResult(request);
               return Json(result, "text/x-json", JsonRequestBehavior.AllowGet);
        ...
        public ActionResult CompanyChanged(int? CompanyList)
        {
            ...
            int? Id = db.Sys_PartnerGroups.Where(t => t.MemberShipId == MemberShipId && t.CompanyId == CompanyList && t.GroupType==1 && t.Deleted != true).Max(c => (int?)c.Id);
        ...
        public JsonResult LoadGroups(int? id, int? CompanyId)
        {
            ...
            var Groups = from e in db.Sys_PartnerGroups
                         where (e.MemberShipId==MemberShipId && e.CompanyId==CompanyId && e.GroupType==1 && e.Deleted != true && (id.HasValue ? e.ParentId == id : e.ParentId == null))
                          select new
                            {
                                id = e.Id,
                                Name = e.NameE,
