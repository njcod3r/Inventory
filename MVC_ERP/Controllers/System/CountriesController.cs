...
namespace MVC_ERP.Controllers
{
    public class CountriesController : BaseController
    {
        ...
        object  Deleted = false;
         ...
         [Authorize]
        public ActionResult Index(int? Id, string submitButton, Sys_Countries MainTbl,int? CompanyListID)
       {
           ...
           switch(submitButton) {
            ...
            default:
               ...
               if (Id == null)
               {
                   Id =  db.Sys_Countries.Where(t => t.MemberShipId == MemberShipId && t.Deleted != true).Max(c => (int?)c.Id);
               }

               if (Id != null)
               {
               MainTbl = db.Sys_Countries.Find(Id);
               }
             
               ModelState.Clear();
               if (MainTbl.Id == 0 || MainTbl.Id == null)
               {
                   Session["MainIdReturnBackToDetailPage"] = 0;
                   Session["MainNameReturnBackToDetailPage"] = "";
               }
               else
               {
                   Session["MainIdReturnBackToDetailPage"] = MainTbl.Id;
                   Session["MainNameReturnBackToDetailPage"] = MainTbl.NameE;
               }
        ...
        // Insert Function
        protected Boolean Insert(Sys_Countries MainTbl) 
            {
                ...
                if (MainTbl.Id == 0)
                {
                 ...
                 if (MainTbl.Code == null)
                 {
                     ...
                     int  Max = db.Database.SqlQuery<int>("select isnull(MAX(CAST(Code AS int)) ,0)+1 from dbo.Sys_Countries  WHERE (isnumeric(Code) = 1) and  (MemberShipId = " + Session["MemberShipId"] + ") AND (ISNULL(Deleted, 0) = 0)").FirstOrDefault(); 
                     MainTbl.Code = Max.ToString();         
                   }

                    MainTbl.Id = ( db.Sys_Countries.Max(c => (int?)c.Id)??0) +1;
        ...
        // Reuse Function
        protected Boolean Reuse(Sys_Countries MainTbl)
        {
            MemberShipId = Convert.ToInt32(Session["MemberShipId"]);
        ...
        protected Boolean Delete(Sys_Countries MainTbl)
        {
            if (MainTbl.Id == null || MainTbl.Id == 0)
            {
                //Message no data to delete
        ...
        protected Boolean Post(Sys_Countries MainTbl)
       {
           if (MainTbl.Id == null || MainTbl.Id == 0)
           {
        ...
        public ActionResult Search([DataSourceRequest]DataSourceRequest request, string TxtSearchVal)
        {
           ...
           {
             var m =  db.Sys_Countries.Where(b => b.MemberShipId == MemberShipId && b.Deleted != true && (b.Code.Contains(TxtSearchVal) || b.NameE.Contains(TxtSearchVal) || b.NameA.Contains(TxtSearchVal) || b.ShortCode.Contains(TxtSearchVal))).Take(20);
             IQueryable<Sys_Countries> mm = m;
             DataSourceResult result = mm.ToDataSourceResult(request);
             return Json(result, "text/x-json", JsonRequestBehavior.AllowGet);
           }
           else
           {
               var m =  db.Sys_Countries.OrderByDescending(b => b.Id).Where(b => b.MemberShipId == MemberShipId && b.Deleted != true).Take(20);
               IQueryable<Sys_Countries> mm = m;
               DataSourceResult result = mm.ToDataSourceResult(request);
               return Json(result, "text/x-json", JsonRequestBehavior.AllowGet);
