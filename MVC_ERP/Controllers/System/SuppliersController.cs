    ...
    public class SuppliersController : BaseController
    {
           ...
           switch(submitButton) {
            ...
            case "Discard":
               //Cancel return to Index view
                   ...
                   Id = db.Sys_Partners.Where(t => t.MemberShipId == MemberShipId && t.CompanyId == CC && t.PartnerType==2 && t.Deleted != true).Max(c => (int?)c.Id);
        ...
        protected Boolean Insert(Sys_Partners MainTbl) 
            {
                ...
                if (MainTbl.Id == 0)
                {
                 ...
                 if (MainTbl.Code == null)
                 {
                     ...
                     int Max = db.Database.SqlQuery<int>("select isnull(MAX(CAST(Code AS int)) ,0)+1 from dbo.Sys_Partners  WHERE (isnumeric(Code) = 1) and CompanyId=(" + MainTbl.CompanyId + ") and PartnerType=2 and   (MemberShipId = " + Session["MemberShipId"] + ") AND (ISNULL(Deleted, 0) = 0)").FirstOrDefault(); 
        ...
        protected Boolean Post(Sys_Partners MainTbl)
       {
        ...
        public ActionResult Search([DataSourceRequest]DataSourceRequest request, string TxtSearchVal, int? CompanySearchId)
        {
           ...
           {
               var m = db.Sys_Partners.Where(b => b.MemberShipId == MemberShipId && b.CompanyId == CompanySearchId && b.PartnerType == 2 && b.Deleted != true && (b.Code.Contains(TxtSearchVal) || b.NameE.Contains(TxtSearchVal) || b.NameA.Contains(TxtSearchVal) || b.Email.Contains(TxtSearchVal))).Take(20);
           ...
           {
               var m = db.Sys_Partners.OrderByDescending(b => b.Id).Where(b => b.MemberShipId == MemberShipId && b.CompanyId == CompanySearchId && b.PartnerType == 2 && b.Deleted != true).Take(20);
               IQueryable<Sys_Partners> mm = m;
               DataSourceResult result = mm.ToDataSourceResult(request);
               return Json(result, "text/x-json", JsonRequestBehavior.AllowGet);
        ...
        public ActionResult CompanyChanged(int? CompanyList)
        {
            ...
            int? Id = db.Sys_Partners.Where(t => t.MemberShipId == MemberShipId && t.CompanyId == CompanyList && t.PartnerType == 2 && t.Deleted != true).Max(c => (int?)c.Id);
